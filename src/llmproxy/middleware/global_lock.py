...
class GlobalLockMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            # For streaming responses, hold the lock until the stream is consumed
            if config.lock.enabled and isinstance(response, StreamingResponse):
                original_body_iterator = getattr(response, "body_iterator", None)
                async def wrapped_body():
                    try:
                        if original_body_iterator is not None:
                            async for chunk in original_body_iterator:
                                yield chunk
                    finally:
                        # Release locks after stream is fully consumed
                        await self._release_locks(backend_name)
                        logger.debug(f"Released locks for {backend_name} after stream completed")
            ...
            # Only release locks if enabled and we actually acquired them
            # (streaming responses release inside the wrapper above)
            if config.lock.enabled and acquired_locks:
                if not (response is not None and isinstance(response, StreamingResponse)):
                    await self._release_locks(backend_name)
